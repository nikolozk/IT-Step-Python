...
def validate_input(prompt, data_type):
    # ვალიდაციის ფუნქცია რათა მოხდეს შემოწმება მომხმარებლის მიერ შეყვანილი სწორი ინფორმაციის
    while True:
        user_input = input(prompt)
        if data_type == "string":
            if user_input.strip(): 
                return user_input
   # თუ კი მომხმარებელი სათაურს და ავტორს სწროად შეიყვანს დაუბრუნებს user_input-ს და გადავა შემდგომ წლის შესაყვანად
...
#პროგრამის მთავარი მენიუ User Interface_ი
def main():
    manager = BookManager()  
